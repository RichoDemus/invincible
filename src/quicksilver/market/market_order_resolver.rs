...
pub fn resolve_orders(
    mut old_orders: Vec<MarketOrder>,
    mut new_order: MarketOrder,
) -> (Vec<MarketOrder>, Vec<Transaction>) {
    let mut transactions = vec![];
    loop {
        let maybe_best_order = old_orders
            .iter_mut()
            .enumerate()
            .filter(|(_, order)| {
        ...
        if amount_to_transfer == 0 {
            panic!(
                "{}",
                format!(
                    "Attempted to do a zero amount transfer: {:?} {:?}",
        ...
        let (seller, seller_price, buyer, buyer_price) = match (&best_order, &new_order) {
            (MarketOrder::SellOrder(seller), MarketOrder::BuyOrder(buyer)) => {
                (seller.seller, seller.price, buyer.buyer, buyer.price)
            }
            (MarketOrder::BuyOrder(buyer), MarketOrder::SellOrder(seller)) => {
                (seller.seller, seller.price, buyer.buyer, buyer.price)
            }
...
mod tests {
    ...
    #[test]
    fn test_no_orders() {
        let (unresolved_orders, transactions) =
            resolve_orders(vec![], SellOrder::from(uuid(0), 100, 10));
    ...
    fn should_not_do_anything_if_no_orders_match() {
        let old_orders = vec![BuyOrder::from(uuid(1), 100, 9)];
    ...
    fn should_return_one_transaction_for_basic_case() {
        let old_orders = vec![BuyOrder::from(uuid(1), 100, 10)];
        let new_order = SellOrder::from(uuid(0), 100, 10);
        ...
        assert_eq!(
            transactions,
            vec![Transaction::from(uuid(0), uuid(1), 100, 10)],
    ...
    fn should_pick_oldest_order_if_same_price() {
        ...
        assert_eq!(
            unresolved_orders,
            vec![BuyOrder::from(uuid(2), 100, 10)],
            "invalid unresolved orders"
        );
        assert_eq!(
            transactions,
    ...
    fn should_partially_consume_existing_order() {
        ...
        assert_eq!(
            unresolved_orders,
            vec![BuyOrder::from(uuid(1), 50, 10)],
            "invalid unresolved orders"
        );
        assert_eq!(
            transactions,
            vec![Transaction::from(uuid(0), uuid(1), 50, 10)],
            "invalid transactions"
        );
    }
    ...
    fn should_partially_consume_new_order() {
        let old_orders = vec![BuyOrder::from(uuid(1), 50, 10)];
        let new_order = SellOrder::from(uuid(0), 100, 10);
        ...
        assert_eq!(
            unresolved_orders,
            vec![SellOrder::from(uuid(0), 50, 10)],
            "invalid unresolved orders"
        );
        assert_eq!(
            transactions,
